    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($recipes as $recipe)
            ...
            <a href="{{ route('recipes.show', $recipe->id) }}" class="block hover:no-underline">
                <div
                    class="flex flex-col rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                    <div class="flex-shrink-0">
                        <img class="h-48 w-full object-cover" src="{{ asset('storage/' . $recipe->image_path) }}"
                             alt="{{ $recipe->name }}">
                    </div>
